...
Calling run() on this object will spawn a subprocess to clone/update the repo
and do the build.

The subprocess runs a shell script that executes the build.

The build process then waits for the build run to
complete, by checking for updates to the build log every 30 seconds.
...
class Kbuild(object):
  ...
  def __start(self):
    ...
    clone = subprocess.Popen([BLDSRV.gce_update_repo,
      self.repository, self.build_dir],
      stdout=f,stderr=f)
